...
public class YouTubeReadonlyClient {
    ...
    private static void printEntry(Entry entry, String prefix) {
        ...
        if(entry.getContent() != null) {
            ...
            System.out.println("\t\t" + prefix + " Content: " + content.getContent().getPlainText());
        }
    ...
    private static void printPlaylists(YouTubeService service,
        ...
        String testUser = promptForUser();
        printPlaylistsFeed(service, USER_FEED_PREFIX + testUser + PLAYLISTS_FEED_SUFFIX, showPlaylistContents);
    }
     ...
     * @throws IOException error sending request or reading the feed.
     */
    public static List<VideoEntry> searchFeed(YouTubeService service, String searchTerms)
            throws IOException, ServiceException {
        YouTubeQuery query = new YouTubeQuery(new URL(VIDEOS_FEED));
        // order results by relevance (most viewed first)
            ...
            try {
                ...
                switch(choice) {
                    ...
                    case 2:
                        ...
                        searchFeed(myService, "radiohead - no surprises");
                        break;
                    case 3:
